...
class NetEaseSmsService extends Model
{
    private $AppKey = '';
    private $AppSecret = '';
    private $RequestType = 'fsockopen';                 //'curl';
    ...
    public function __construct(){
        $this->AppKey = C('net_AppKey');
        $this->AppSecret = C('net_AppSecret');
    }
     ...
     */
    public function sendSmsCode($templateid, $mobile, $deviceId = '')
    {
    ...
    public function sendSMSTemplate($templateid, $mobiles = array(), $params = '')
    {
        ...
        $data = array(
            ...
            //'params' => json_encode(["贝拉小镇","5869","测试"])
        );
        if ($this->RequestType == 'curl') {
            $result = $this->postDataCurl($url, $data);
        } else {

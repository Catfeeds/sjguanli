...
//    }
    ...
    public function main(){
        ...
        $arr = D('Goods')->alias('g')
            ...
            sum(case when o.`status` = 3 and o.is_balance = 1 and check_time < '.$ctime.' then o.total_price else 0 end)as sum3,
        ...
        $kgoods = D('Goods')->alias('g')
            ->field('g.goods_id,g.title,
                (SELECT sum(case when o.`status`=3 and o.is_balance=1 and o.check_time<'.$ktime.' then o.total_price else 0 end) from practice_order as o
                left join practice_order_goods as og on og.order_id = o.order_id
                where og.goods_id = g.goods_id)as sum')
            ->where(array('g.is_delete'=>0,'g.status'=>4,'g.shop_id'=>$shop_id))->limit(3)->select();

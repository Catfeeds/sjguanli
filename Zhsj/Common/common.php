...
function setUid($uid){
    ...
    cookie('BAOCMS_TOKEN',$uid,86400*365); //存一年
    return true;
}
function clearUid(){
    cookie('BAOCMS_TOKEN',null); //存一年
    return true;
}
...
//设置代理商用户编号
function setProxyUid($uid){
    import("ORG/Crypt/Base64");
    $uid = 'PROXY_USER_'.$uid.'_'.NOW_TIME;
    $uid = Base64::encrypt($uid, C('AUTH_KEY'));
    cookie('PROXY_TOKEN',$uid,86400 * 7); //存一周
    return true;
}

//清除代理商用户编号
function clearProxyUid(){
    cookie('PROXY_TOKEN',null);
    return true;
}

//获取代理商用户编号
function getProxyUid(){
    import("ORG/Crypt/Base64");
    $token = cookie('PROXY_TOKEN');
    $token = Base64::decrypt($token, C('AUTH_KEY'));
    $token = explode('_', $token);
    if($token[0]!= 'PROXY') return 0;
    return (int)$token[2];
}

//设置商家用户编号
function setShopUid($uid){
    import("ORG/Crypt/Base64");
    $uid = 'SHOP_USER_'.$uid.'_'.NOW_TIME;
    $uid = Base64::encrypt($uid, C('AUTH_KEY'));
    cookie('SHOP_TOKEN',$uid,86400 * 7); //存一周
    return true;
}

//清除商家用户编号
function clearShopUid(){
    cookie('SHOP_TOKEN',null);
    return true;
}
...
function getRefundType(){
    return array(
        0=>array('id'=>1,'name'=>'不可退，不可改'),
        1=>array('id'=>2,'name'=>'活动开始前一天不可退不可改'),
    );
}

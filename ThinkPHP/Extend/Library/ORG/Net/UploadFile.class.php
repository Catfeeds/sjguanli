...
class UploadFile {//类定义开始
     ...
     * @access public
     * @param mixed $name 数据
     * @param int $flag 用于设置图片缩略图大小的参数
     * @param string $value  数据表名
     * @return string
     */
    private function save($file ,$flag) {
        ...
        if($this->thumb && in_array(strtolower($file['extension']),array('gif','jpg','jpeg','bmp','png'))) {
            ...
            if(false !== $image) {
                ...
                for($i=0,$len=count($thumbWidth); $i<$len; $i++) {
                    ...
                    if(1 == $this->thumbType){
                        Image::thumb2($filename,$thumbPath.$thumbname.'.'.$thumbExt,'',$thumbWidth[$i],$thumbHeight[$i],true,$flag);
                    }else{
     ...
     * 上传所有文件
     * @access public
     * @param string $flag 判断上传图片是否是头像
     * @param string $savePath  上传文件保存路径
     * @return string
     */
    public function upload($flag ,$savePath ='') {
        //如果不指定保存文件名，则由系统默认
        if(empty($savePath))
        ...
        foreach($files as $key => $file) {
            ...
            if(!empty($file['name'])) {
                ...
                //保存上传文件
                if(!$this->save($file,$flag)) return false;

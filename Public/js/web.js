...
function bmsg(msg, url, timeout, callback) { //信息,跳转地址,时间
    jctip(msg, url, timeout, callback);
}
 ...
 *  实践平台FROM提交表单统一提示框
 * */
function jctip(msg, url, timeout, callback) {
    $('.poptip').text(msg);
    $('.poptip').fadeIn(200);
    $('.popmask').fadeIn(200);
    if (url) {
 ...
 *  确认提示框JS方法
 * */
function jcconfirm(txt,callback) {
    $('.jcconfirm').show();
    $('.popmask').show();
    $('.jcconfirm p').text(txt);
    $('.jcconfirm .jcconfirmbtn').on('click',function () {
        $('.jcconfirm').hide();
        $('.popmask').hide();
